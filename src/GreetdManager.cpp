...
{
    qDebug() << "authenticate: " << username << password;
    mErrorMsg.clear();
...
{
    /** Get the log name */
    QString logName = logPath + "/session/QtGreet-"
        + QDateTime::currentDateTime().toString("ddMMyyyy-hhmmss") + ".log";
...
QString GreetdLogin::getX11Session(QString base)
{
    ...
    QString hash = QCryptographicHash::hash(QDateTime::currentDateTime().toString().toUtf8(),
...
QByteArray GreetdLogin::roundtrip(QByteArray payload)
{
    ...
    char message[200] = { '\0' }; // TODO:(rewine)
